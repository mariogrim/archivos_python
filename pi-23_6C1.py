...
def desafio():
    ...
    cantidad = 0
    while True:
        producto = input('Ingrese producto a modificar: ').lower()
        if producto == 'fin':
            print('Finaliza el ingreso')
            break
        elif producto in stock:
            cantidad = int(input('Ingrese la cantidad a agregar: '))
            stock[producto] += cantidad
            print('La nueva cantidad para ', producto, ' es ', cantidad)
        else:
            print('El producto ingresado no es parte del stock')        
    print('Las nuevas cantidades del stock son:\n', stock)
    return stock
